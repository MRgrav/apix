...
RUN apt-get update && apt-get install -y \
    ...
    && docker-php-ext-install pdo pdo_pgsql mbstring zip exif pcntl bcmath sockets intl gmp gd

# Swoole is installed from GitHub
RUN curl -L -o swoole.tar.gz https://github.com/swoole/swoole-src/archive/refs/tags/v6.1.0.tar.gz \
...
RUN composer install --no-dev --optimize-autoloader --no-interaction --prefer-dist --no-scripts -vvv
